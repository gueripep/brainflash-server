...
from app.pydantic.flashcard import FlashcardFSRSSchema, create_flashcard_fsrs_orm_from_dto
...
@router.get("/fsrs/", response_model=list[FlashcardFSRSSchema])
async def read_fsrs(offset: int = 0, limit: int = Query(default=100, le=100), session: AsyncSession = Depends(get_db)):
    ...
    pydantic_fsrs = [FlashcardFSRSSchema.model_validate(fsr) for fsr in fsrs]
    return pydantic_fsrs

@router.get("{flashcard_id}/fsrs/", response_model=FlashcardFSRSSchema)
async def read_fsrs(flashcard_id: str, session: AsyncSession = Depends(get_db)):
    ...
    return FlashcardFSRSSchema.model_validate(db_fsrs)


@router.put("/{flashcard_id}/fsrs", response_model=FlashcardFSRSSchema)
async def update_fsrs(flashcard_id: str, payload: FlashcardFSRSSchema, session: AsyncSession = Depends(get_db)):
    """
    ...
    pydantic_fsrs = FlashcardFSRSSchema.model_validate(payload)
    
    new_db_fsrs = create_flashcard_fsrs_orm_from_dto(pydantic_fsrs)
